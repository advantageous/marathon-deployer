...
/**
 * Deploy the application to Marathon.
 *
 * @param options command options
 * @param config  the marathon.json config
 * @param pkg     the package config for the application being deployed.
 */
function deploy(options, config, pkg) {

  const transport = require(options.url.startsWith('https') ? 'https' : 'http');
  const uri = require('url').parse(options.url);
  ...
  const listRequest = transport.get(Object.assign(uri, {path: basePath + 'v2/apps'}), (res) => {
    ...
    res.on('end', () => {
      if (res.statusCode == 200) {
        ...
        const payload = JSON
          .stringify(config)
          .replace(new RegExp('__IMAGE__', 'g'), options.image)
